...
/// Native HLS Video Player - YouTube-like Experience
/// Fullscreen handled via Overlay to prevent controller disposal
...
class _NativeHLSPlayerState extends State<NativeHLSPlayer> with WidgetsBindingObserver {
  ...
  Future<void> _initializePlayer() async {
    try {
      debugPrint('🎬 Initializing HLS player: ${widget.hlsUrl}');
      ...
      debugPrint('✅ Video initialized: ${_controller!.value.duration}');
                  ...
                  child: Column(
                    children: [
                        ...
                        child: Row(
                          ...
                          children: [
                            _ctrlBtn(Icons.replay_10, () {
                              _seekTo(pos - const Duration(seconds: 10));
                            }, 40),
                            const SizedBox(width: 48),
                            _ctrlBtn(playing ? Icons.pause : Icons.play_arrow, _togglePlayPause, 56),
                            const SizedBox(width: 48),
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            Text(_formatTime(pos), style: const TextStyle(color: Colors.white, fontSize: 13)),
                            ...
                            const SizedBox(width: 12),
                            Text(_formatTime(dur), style: const TextStyle(color: Colors.white, fontSize: 13)),
          ...
          child: Column(
            ...
            children: [
              ...
              ...([0.5, 0.75, 1.0, 1.25, 1.5, 2.0]).map((s) => ListTile(
                ...
                title: Text(
                  s == 1.0 ? 'Normal' : '${s}x',
                  style: TextStyle(color: _playbackSpeed == s ? Colors.blue : Colors.white),
                ),
                trailing: _playbackSpeed == s ? const Icon(Icons.check, color: Colors.blue, size: 20) : null,
                onTap: () {
      ...
      child: Stack(
        ...
        children: [
          ...
          if (_isBuffering)
            const Center(child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)),
          // Controls
          if (_showControls) _buildControls(pos, dur, playing, posMs, durMs),
        ],
      ),
    );
  }

  Widget _buildControls(Duration pos, Duration dur, bool playing, double posMs, double durMs) {
    return Container(
      color: Colors.black38,
      child: Column(
        children: [
          ...
          Expanded(
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                _buildControlBtn(Icons.replay_10, () => _seekTo(pos - const Duration(seconds: 10)), 28),
                const SizedBox(width: 24),
                _buildControlBtn(playing ? Icons.pause : Icons.play_arrow, _togglePlayPause, 44),
                const SizedBox(width: 24),
                _buildControlBtn(Icons.forward_10, () => _seekTo(pos + const Duration(seconds: 10)), 28),
              ],
            ),
          ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                Text(_formatTime(pos), style: const TextStyle(color: Colors.white, fontSize: 10)),
                Expanded(
                  child: SliderTheme(
                    data: SliderTheme.of(context).copyWith(
                      trackHeight: 2,
                ...
                Text(_formatTime(dur), style: const TextStyle(color: Colors.white, fontSize: 10)),
  ...
  Widget _buildControlBtn(IconData icon, VoidCallback onTap, double size) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Failed to load', style: TextStyle(color: Colors.white, fontSize: 13)),
            const SizedBox(height: 4),
            Text(_errorMessage ?? '', style: const TextStyle(color: Colors.white54, fontSize: 10), textAlign: TextAlign.center, maxLines: 2),
            const SizedBox(height: 8),
            ElevatedButton(
              onPressed: () {
      ...
      child: Column(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Replay', style: TextStyle(color: Colors.white, fontSize: 12)),
